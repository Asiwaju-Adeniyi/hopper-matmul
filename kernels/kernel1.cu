...
namespace M1 {
    ...
    __device__ static inline uint64_t matrixPropEncoder(uint64_t x) {return (((x) & 0x3FFFF)) >> 0x4;} /*This helper takes in a 64bit int keeps the last 18bits in the least significant side
        and then right-shifts it by 4 bits.*/

    __device uint64_t smemDescriptor(uint32_t *ptr) {
        ...
        desc |= matrixPropEncoder(16) << 16; //combiners the leading dimension bytes offset from bit 16
